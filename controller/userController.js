...
exports.register = async (req,res)=>{
    try {
        const userExists = await User.findOne({email:req.body.email}) //single parameter
...
exports.login = async (req,res)=>{
    try {
        ...
        if(!validPassword) return res.status(400).json({errors:true,message:"email or passward is invalid"})
